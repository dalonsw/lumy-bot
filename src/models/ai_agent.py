...
class AIAgent:
    def __init__(self, nome: str, token_file: str = "puter_token.pkl", 
                 policy_file: str = "src/config/prompt_policy.json", 
                 model: str = "gpt-5-chat-latest", 
                 max_tokens: int = 100, ultima_mensagem: json = []):
        self.__nome = nome
        self.__cliente = PuterClient()
        self.__token_file = token_file
        self.__prompt_policy = json.load(open(policy_file, "r", 
                                              encoding="utf-8"))
        self.__model = model
    ...
    def sincronizar_datetime_atual(self):
        return [{
            "role": "system",
            "content": f"A data e hora atual é "
                       f"{datetime.datetime.now()}."
        }]

    def salvar_ultima_mensagem(self, mensagem_usuario: str, 
                               mensagem_assistente: str):
        self.__ultima_mensagem = [
            {
                "role": "user", 
                "content": f"A última mensagem enviada pelo usuário " +
                           f"foi: {mensagem_usuario}"
            },
            {
                "role": "assistant", 
                "content": f"A última resposta do assistente foi: " +
                           f"{mensagem_assistente}"
            }
        ]
    ...
    def conectar(self):
        if os.path.exists(self.__token_file):
            ...
            self.__cliente.login(os.getenv("CLIENT_NAME"), 
    ...
    def perguntar(self, pergunta: str,):
        ...
        response = self.__cliente.ai_chat(
            messages=messages,
            options={"model": self.__model, "temperature": 1, 
